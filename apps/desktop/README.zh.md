...
替换现有安装前，NSIS 安装程序会请求正在运行的单实例进入普通显式退出路径，并等待受 supervisor 管理的 Host 完全停稳。随后，自定义的运行中应用检查会终止任何残留的 `DeepSeek Harness.exe` 进程树，并替换 Electron Builder 的通用提示循环，因此常驻托盘或较旧的应用既无法继续锁定安装目录，也不会再要求人工重试。
